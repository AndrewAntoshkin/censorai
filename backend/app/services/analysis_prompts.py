...
VIDEO_ANALYSIS_PROMPT = """Ты — профессиональный цензор видеоконтента для российского рынка. Проанализируй видео ПОЛНОСТЬЮ — от первого до последнего кадра, без пропусков. Поле duration в ответе должно отражать реальную длину всего загруженного файла, а не только первых минут.
